...
# DataScraper portion of the NintendoWebScrape project
# Before running the datascraper, you must run linkscraper.py at least once
# This file has two functions a user can run: new_link_data_scraper() & entire_data_scraper()
...
# Scrape only data for links that haven't been scraped yet --INCOMPLETE--
def new_link_data_scraper():
    ...
    category_name_list = sorted(category_parser())
    ...
    for platform in game_link_df:
        ...
        for link in game_link_processed:
            if link not in analyzed_links_s.values:
                attempts = 1
                link_data_dict = page_parser(link, category_name_list)
                print('\n \n')
                while link_data_dict['Name'] == 'ERROR' and attempts <= 10:
                    print("ATTEMPT: ", attempts)
                    attempts += 1
                    link_data_dict = page_parser(url, category_name_list)
                if attempts == 11:
